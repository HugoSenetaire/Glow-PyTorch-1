...
def save_figures(output_path, input1, input2, prefix, dataset1_name, dataset2_name):
    for key in input1.keys():
        ...
        plt.xlabel(f"{prefix}")
        plt.hist(input1[key], label=dataset1_name, density=True, bins=50, alpha =0.8, color ='r')
        plt.hist(input2[key], label=dataset2_name, density=True, bins=30, alpha = 0.8, color = 'b')
        plt.legend()
